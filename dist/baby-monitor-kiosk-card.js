const U = globalThis, q = U.ShadowRoot && (U.ShadyCSS === void 0 || U.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, G = /* @__PURE__ */ Symbol(), et = /* @__PURE__ */ new WeakMap();
let mt = class {
  constructor(t, s, i) {
...
const St = (e) => new mt(typeof e == "string" ? e : e + "", void 0, G), gt = (e, ...t) => {
...
}, st = q ? (e) => e : (e) => e instanceof CSSStyleSheet ? ((t) => {
  let s = "";
  for (const i of t.cssRules) s += i.cssText;
  return St(s);
})(e) : e;
const { is: Ct, defineProperty: kt, getOwnPropertyDescriptor: xt, getOwnPropertyNames: Ot, getOwnPropertySymbols: Mt, getPrototypeOf: Pt } = Object, j = globalThis, it = j.trustedTypes, Nt = it ? it.emptyScript : "", It = j.reactiveElementPolyfillSupport, k = (e, t) => e, T = { toAttribute(e, t) {
  switch (t) {
    case Boolean:
      e = e ? Nt : null;
      break;
  ...
  return s;
} }, K = (e, t) => !Ct(e, t), ot = { attribute: !0, type: String, converter: T, reflect: !1, useDefault: !1, hasChanged: K };
...
let y = class extends HTMLElement {
  ...
  static finalize() {
    ...
    if (this.finalized = !0, this._$Ei(), this.hasOwnProperty(k("properties"))) {
      const s = this.properties, i = [...Ot(s), ...Mt(s)];
  ...
  static finalizeStyles(t) {
    ...
    if (Array.isArray(t)) {
      ...
      for (const o of i) s.unshift(st(o));
  ...
  _$ET(t, s) {
    ...
    if (o !== void 0 && i.reflect === !0) {
      const n = (i.converter?.toAttribute !== void 0 ? i.converter : T).toAttribute(s, i.type);
  ...
  _$AK(t, s) {
    ...
    if (o !== void 0 && this._$Em !== o) {
      const n = i.getPropertyOptions(o), r = typeof n.converter == "function" ? { fromAttribute: n.converter } : n.converter?.fromAttribute !== void 0 ? n.converter : T;
      this._$Em = o;
  ...
  requestUpdate(t, s, i, o = !1, n) {
    if (t !== void 0) {
      ...
      if (o === !1 && (n = this[t]), i ??= r.getPropertyOptions(t), !((i.hasChanged ?? K)(n, s) || i.useDefault && i.reflect && n === this._$Ej?.get(t) && !this.hasAttribute(r._$Eu(t, i)))) return;
      this.C(t, s, i);
    }
...
y.elementStyles = [], y.shadowRootOptions = { mode: "open" }, y[k("elementProperties")] = /* @__PURE__ */ new Map(), y[k("finalized")] = /* @__PURE__ */ new Map(), It?.({ ReactiveElement: y }), (j.reactiveElementVersions ??= []).push("2.1.2");
const J = globalThis, nt = (e) => e, R = J.trustedTypes, rt = R ? R.createPolicy("lit-html", { createHTML: (e) => e }) : void 0, _t = "$lit$", g = `lit$${Math.random().toFixed(9).slice(2)}$`, $t = "?" + g, Lt = `<${$t}>`, b = document, O = () => b.createComment(""), M = (e) => e === null || typeof e != "object" && typeof e != "function", F = Array.isArray, Ut = (e) => F(e) || typeof e?.[Symbol.iterator] == "function", V = `[ 	
\f\r]`, C = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, at = /-->/g, ht = />/g, _ = RegExp(`>|${V}(?:([^\\s"'>=/]+)(${V}*=${V}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), lt = /'/g, ct = /"/g, bt = /^(?:script|style|textarea|title)$/i, Dt = (e) => (t, ...s) => ({ _$litType$: e, strings: t, values: s }), p = Dt(1), E = /* @__PURE__ */ Symbol.for("lit-noChange"), l = /* @__PURE__ */ Symbol.for("lit-nothing"), dt = /* @__PURE__ */ new WeakMap(), $ = b.createTreeWalker(b, 129);
function yt(e, t) {
...
const Tt = (e, t) => {
  ...
  for (let h = 0; h < s; h++) {
    const a = e[h];
    let d, u, c = -1, f = 0;
    for (; f < a.length && (r.lastIndex = f, u = r.exec(a), u !== null); ) f = r.lastIndex, r === C ? u[1] === "!--" ? r = at : u[1] !== void 0 ? r = ht : u[2] !== void 0 ? (bt.test(u[2]) && (o = RegExp("</" + u[2], "g")), r = _) : u[3] !== void 0 && (r = _) : r === _ ? u[0] === ">" ? (r = o ?? C, c = -1) : u[1] === void 0 ? c = -2 : (c = r.lastIndex - u[2].length, d = u[1], r = u[3] === void 0 ? _ : u[3] === '"' ? ct : lt) : r === ct || r === lt ? r = _ : r === at || r === ht ? r = C : (r = _, o = void 0);
    const m = r === _ && e[h + 1].startsWith("/>") ? " " : "";
    n += r === C ? a + Lt : c >= 0 ? (i.push(d), a.slice(0, c) + _t + a.slice(c) + g + m) : a + g + (c === -2 ? h : m);
  }
  return [yt(e, n + (e[s] || "<?>") + (t === 2 ? "</svg>" : t === 3 ? "</math>" : "")), i];
};
class P {
  constructor({ strings: t, _$litType$: s }, i) {
    ...
    this.parts = [];
    let n = 0, r = 0;
    const h = t.length - 1, a = this.parts, [d, u] = Tt(t, s);
    if (this.el = P.createElement(d, i), $.currentNode = this.el.content, s === 2 || s === 3) {
    ...
    for (; (o = $.nextNode()) !== null && a.length < h; ) {
      if (o.nodeType === 1) {
        if (o.hasAttributes()) for (const c of o.getAttributeNames()) if (c.endsWith(_t)) {
          const f = u[r++], m = o.getAttribute(c).split(g), L = /([.?@])?(.*)/.exec(f);
          a.push({ type: 1, index: n, name: L[2], strings: m, ctor: L[1] === "." ? Ht : L[1] === "?" ? jt : L[1] === "@" ? Bt : B }), o.removeAttribute(c);
        ...
        if (bt.test(o.tagName)) {
          ...
          if (f > 0) {
            ...
            for (let m = 0; m < f; m++) o.append(c[m], O()), $.nextNode(), a.push({ type: 2, index: ++n });
            o.append(c[f], O());
          }
        }
        ...
        let c = -1;
        for (; (c = o.data.indexOf(g, c + 1)) !== -1; ) a.push({ type: 7, index: n }), c += g.length - 1;
      }
      n++;
    }
  }
...
function S(e, t, s = e, i) {
  ...
  return o?.constructor !== n && (o?._$AO?.(!1), n === void 0 ? o = void 0 : (o = new n(e), o._$AT(e, s, i)), i !== void 0 ? (s._$Co ??= [])[i] = o : s._$Cl = o), o !== void 0 && (t = S(e, o._$AS(e, t.values), o, i)), t;
}
class Rt {
  constructor(t, s) {
  ...
  u(t) {
    const { el: { content: s }, parts: i } = this._$AD, o = (t?.creationScope ?? b).importNode(s, !0);
    $.currentNode = o;
    let n = $.nextNode(), r = 0, h = 0, a = i[0];
    for (; a !== void 0; ) {
      if (r === a.index) {
        let d;
        a.type === 2 ? d = new I(n, n.nextSibling, this, t) : a.type === 1 ? d = new a.ctor(n, a.name, a.strings, this, t) : a.type === 6 && (d = new zt(n, this, t)), this._$AV.push(d), a = i[++h];
      }
      r !== a?.index && (n = $.nextNode(), r++);
    }
    return $.currentNode = b, o;
  }
  p(t) {
    let s = 0;
...
class I {
  ...
  constructor(t, s, i, o) {
    this.type = 2, this._$AH = l, this._$AN = void 0, this._$AA = t, this._$AB = s, this._$AM = i, this.options = o, this._$Cv = o?.isConnected ?? !0;
  }
  get parentNode() {
  ...
  _$AI(t, s = this) {
    t = S(this, t, s), M(t) ? t === l || t == null || t === "" ? (this._$AH !== l && this._$AR(), this._$AH = l) : t !== this._$AH && t !== E && this._(t) : t._$litType$ !== void 0 ? this.$(t) : t.nodeType !== void 0 ? this.T(t) : Ut(t) ? this.k(t) : this._(t);
  }
  O(t) {
  ...
  _(t) {
    this._$AH !== l && M(this._$AH) ? this._$AA.nextSibling.data = t : this.T(b.createTextNode(t)), this._$AH = t;
  }
  $(t) {
    const { values: s, _$litType$: i } = t, o = typeof i == "number" ? this._$AC(t) : (i.el === void 0 && (i.el = P.createElement(yt(i.h, i.h[0]), this.options)), i);
  ...
  _$AC(t) {
    ...
    return s === void 0 && dt.set(t.strings, s = new P(t)), s;
  }
  k(t) {
    F(this._$AH) || (this._$AH = [], this._$AR());
    const s = this._$AH;
    let i, o = 0;
    for (const n of t) o === s.length ? s.push(i = new I(this.O(O()), this.O(O()), this, this.options)) : i = s[o], i._$AI(n), o++;
...
class B {
  ...
  constructor(t, s, i, o, n) {
    this.type = 1, this._$AH = l, this._$AN = void 0, this.element = t, this.name = s, this._$AM = o, this.options = n, i.length > 2 || i[0] !== "" || i[1] !== "" ? (this._$AH = Array(i.length - 1).fill(new String()), this.strings = i) : this._$AH = l;
  }
  _$AI(t, s = this, i, o) {
    const n = this.strings;
    let r = !1;
    if (n === void 0) t = S(this, t, s, 0), r = !M(t) || t !== this._$AH && t !== E, r && (this._$AH = t);
    else {
      const h = t;
      let a, d;
      for (t = n[0], a = 0; a < n.length - 1; a++) d = S(this, h[i + a], s, a), d === E && (d = this._$AH[a]), r ||= !M(d) || d !== this._$AH[a], d === l ? t = l : t !== l && (t += (d ?? "") + n[a + 1]), this._$AH[a] = d;
    }
    r && !o && this.j(t);
  }
  j(t) {
...
class Bt extends B {
  ...
  _$AI(t, s = this) {
    if ((t = S(this, t, s, 0) ?? l) === E) return;
    const i = this._$AH, o = t === l && i !== l || t.capture !== i.capture || t.once !== i.once || t.passive !== i.passive, n = t !== l && (i === l || o);
...
Vt?.(P, I), (J.litHtmlVersions ??= []).push("3.3.3");
const vt = (e, t, s) => {
  const i = s?.renderBefore ?? t;
  let o = i._$litPart$;
  if (o === void 0) {
    const n = s?.renderBefore ?? null;
    i._$litPart$ = o = new I(t.insertBefore(O(), n), n, void 0, s ?? {});
  }
  return o._$AI(e), o;
};
const Q = globalThis;
class A extends y {
  ...
  update(t) {
    const s = this.render();
    this.hasUpdated || (this.renderOptions.isConnected = this.isConnected), super.update(t), this._$Do = vt(s, this.renderRoot, this.renderOptions);
  }
  connectedCallback() {
...
const Wt = Q.litElementPolyfillSupport;
Wt?.({ LitElement: A });
...
const qt = { attribute: !0, type: String, converter: T, reflect: !1, hasChanged: K }, Gt = (e = qt, t, s) => {
...
function Z(e) {
  return (t, s) => typeof s == "object" ? Gt(e, t, s) : ((i, o, n) => {
...
const Et = "baby-monitor-kiosk-card:", Y = (e) => Et + e;
function D(e) {
  return localStorage.getItem(Y(e)) === "paired";
}
function Kt(e) {
  localStorage.setItem(Y(e), "paired");
}
function Jt(e) {
...
let N = class extends A {
  ...
  entity(e, t, s, i) {
    return p`<label>${t}<ha-entity-picker .hass=${this.hass} .value=${s} .includeDomains=${i} allow-custom-entity @value-changed=${(o) => this.set(e, o.detail.value)}></ha-entity-picker></label>`;
  }
  render() {
    const e = this.config;
    if (!e) return p``;
    const t = e.id ?? `${e.camera ?? e.stream}|${e.sound_sensor}`, s = (i, o, n) => p`<label><input type=checkbox .checked=${n} @change=${(r) => this.set(i, r.target.checked)}> ${o}</label>`;
    return p`<div class=grid><label>Naam/ID<input .value=${e.id ?? ""} @change=${(i) => this.set("id", i.target.value)}></label>${this.entity("camera", "Camera entity", e.camera ?? "", ["camera"])}<label>go2rtc/WebRTC stream<input .value=${e.stream ?? ""} @change=${(i) => this.set("stream", i.target.value)}></label>${this.entity("sound_sensor", "Geluidsensor", e.sound_sensor ?? "", ["sensor", "binary_sensor"])}${[["sound_threshold_db", "Activeringsgrens dB"], ["sound_reset_db", "Resetgrens dB"], ["trigger_for", "Minimale geluidsduur (s)"], ["silence_duration", "Stiltetijd (s)"], ["manual_close_cooldown", "Cooldown handmatig sluiten (s)"]].map(([i, o]) => p`<label>${o}<input type=number .value=${String(e[i] ?? "")} @change=${(n) => this.set(i, Number(n.target.value))}></label>`)}${s("preload", "Stream voorverwarmen", e.preload !== !1)}${s("auto_open", "Automatisch openen", e.auto_open !== !1)}${s("kiosk.enabled", "Kioskmodus", e.kiosk?.enabled !== !1)}${s("kiosk.device_bound", "Alleen gekoppeld apparaat", e.kiosk?.device_bound !== !1)}<label>Toegestane HA-user-ID’s (komma-gescheiden)<input .value=${e.kiosk?.allowed_user_ids?.join(", ") ?? ""} @change=${(i) => this.set("kiosk.allowed_user_ids", i.target.value.split(",").map((o) => o.trim()).filter(Boolean))}></label>${s("audio.default_muted", "Standaard gedempt", e.audio?.default_muted ?? !1)}${s("audio.remember_state", "Audiostatus onthouden", e.audio?.remember_state !== !1)}${s("controls.mute", "Mute-knop tonen", e.controls?.mute !== !1)}${s("controls.close", "Sluitknop tonen", e.controls?.close !== !1)}${s("controls.volume", "Volumeknoppen tonen", e.controls?.volume ?? !1)}${s("controls.brightness", "Helderheidsknoppen tonen", e.controls?.brightness ?? !1)}${s("debug", "Debugstatus", !!e.debug)}<label>Companion notify service<input .value=${e.companion?.notify_service ?? ""} @change=${(i) => this.set("companion.notify_service", i.target.value)}></label>${[["active_brightness", "Actieve helderheid"], ["idle_brightness", "Rusthelderheid"], ["active_volume", "Actief volume"], ["idle_volume", "Rustvolume"]].map(([i, o]) => p`<label>${o}<input type=number min=0 max=100 .value=${String(e.companion?.[i] ?? "")} @change=${(n) => this.set(`companion.${i}`, Number(n.target.value))}></label>`)}<div class=pair><b>Dit apparaat gekoppeld: ${D(t) ? "ja" : "nee"}</b><button @click=${() => {
      D(t) ? Jt(t) : Kt(t), this.requestUpdate();
    }}>${D(t) ? "Ontkoppel" : "Koppel dit apparaat"}</button></div></div>`;
  }
};
N.styles = gt`:host{display:block;padding:16px}.grid{display:grid;gap:12px}label{display:grid;gap:4px}input{padding:8px}.pair{display:flex;gap:8px;align-items:center}button{padding:10px}`;
tt([
  Z({ attribute: !1 })
], N.prototype, "hass", 2);
tt([
...
N = tt([
  At("baby-monitor-kiosk-card-editor")
], N);
const v = { preload: !0, trigger_for: 1, silence_duration: 5, manual_close_cooldown: 30, sound_threshold_db: 48, sound_reset_db: 42 };
function Zt(e, t) {
  switch (e) {
    case "IDLE":
...
class Yt {
  constructor(t, s) {
    this.config = t, this.key = s, this.listeners = /* @__PURE__ */ new Set(), this.lastLoud = !1, this.state = "IDLE", this.machine = new Xt((t.trigger_for ?? v.trigger_for) * 1e3, (t.silence_duration ?? v.silence_duration) * 1e3, (t.manual_close_cooldown ?? v.manual_close_cooldown) * 1e3, (i) => {
  ...
  update(t) {
    ...
    let n;
    if (i)
      n = this.lastLoud ? o > (this.config.sound_reset_db ?? v.sound_reset_db) : o >= (this.config.sound_threshold_db ?? v.sound_threshold_db);
    else {
...
const H = /* @__PURE__ */ new Map(), pt = /* @__PURE__ */ new Map();
function te(e) {
  ...
  let i = H.get(t);
  return (!i || pt.get(t) !== s) && (i = new Yt(e, t), H.set(t, i), pt.set(t, s)), i;
}
const x = /* @__PURE__ */ new Map();
function W(e, t) {
  return x.has(e) || x.set(e, t), x.get(e) === t;
}
function ft(e, t) {
...
let w = class extends A {
  constructor() {
    super(...arguments), this.machineState = "IDLE", this.muted = !0, this.cameraSignature = "", this.currentBrightness = 100, this.currentVolume = 100;
  }
  ...
  setConfig(e) {
    ...
    this.config && this.configId !== t && (this.unsubscribe?.(), ft(this.configId, this), this.portal?.remove(), this.portal = void 0, this.camera = void 0, this.cameraSignature = ""), this.config = { ...v, ...e }, this.id = this.configId, this.currentBrightness = e.companion?.active_brightness ?? 100, this.currentVolume = e.companion?.active_volume ?? 100;
  ...
  eligible() {
    return this.config.kiosk?.enabled !== !1 && (this.config.kiosk?.device_bound === !1 || D(this.configId)) && (!this.config.kiosk?.allowed_user_ids?.length || !!this.hass?.user && this.config.kiosk.allowed_user_ids.includes(this.hass.user.id));
  }
  active() {
  ...
  syncPortal() {
    ...
    this.portal.style.cssText = e ? "position:fixed;inset:0;z-index:2147483000;background:#000;display:grid;place-items:center;" : "position:fixed;width:2px;height:2px;opacity:.01;pointer-events:none;overflow:hidden;left:-4px;top:-4px;", vt(e || t ? p`<style>.cam{width:100%;height:100%}.controls{position:absolute;left:50%;bottom:max(24px,env(safe-area-inset-bottom));transform:translateX(-50%);display:flex;gap:12px;flex-wrap:wrap;justify-content:center}.controls button{width:68px;height:68px;border:0;border-radius:50%;font-size:25px;background:#111c;color:#fff}</style><div class=cam>${this.camera}</div>${e ? p`<div class=controls>${this.config.controls?.brightness ? p`<button aria-label="Helderheid lager" @click=${() => this.kioskLevel("brightness", -20)}>☀−</button><button aria-label="Helderheid hoger" @click=${() => this.kioskLevel("brightness", 20)}>☀+</button>` : l}${this.config.controls?.volume ? p`<button aria-label="Volume lager" @click=${() => this.kioskLevel("volume", -20)}>−🔊</button><button aria-label="Volume hoger" @click=${() => this.kioskLevel("volume", 20)}>+🔊</button>` : l}${this.config.controls?.mute !== !1 ? p`<button aria-label=${this.muted ? "Geluid aan" : "Geluid uit"} @click=${() => this.toggleMute()}>${this.muted ? "🔇" : "🔊"}</button>` : l}${this.config.controls?.close !== !1 ? p`<button aria-label="Sluiten" @click=${() => this.runtime?.close()}>✕</button>` : l}</div>` : l}` : l, this.portal);
  }
  render() {
    if (!this.config) return l;
    const e = this.hass?.states[this.config.sound_sensor]?.state ?? "?";
    return p`${!this.active() && this.config.preload === !1 ? p`<span class=idle>Baby monitor gereed</span>` : l}${this.config.debug ? p`<div class=debug>state=${this.machineState} | value=${e} | on≥${this.config.sound_threshold_db ?? "-"} | off≤${this.config.sound_reset_db ?? "-"} | timer=${Math.ceil((this.runtime?.remainingMs() ?? 0) / 1e3)}s | kiosk=${this.eligible()} | preload=${this.config.preload !== !1 && this.eligible()}</div>` : l}`;
  }
};
w.styles = gt`:host{display:block;min-height:1px}.stage{position:fixed;inset:0;z-index:2147483000;background:#000;display:grid;place-items:center}.stage>*:first-child{width:100%;height:100%}.controls{position:absolute;left:50%;bottom:max(24px,env(safe-area-inset-bottom));transform:translateX(-50%);display:flex;gap:18px}.controls button{width:68px;height:68px;border:0;border-radius:50%;font-size:28px;background:#111c;color:white}.preload{position:fixed;width:2px;height:2px;opacity:.01;pointer-events:none;overflow:hidden;left:-4px;top:-4px}.debug{position:fixed;z-index:2147483001;top:8px;left:8px;background:#000b;color:#fff;padding:8px;font:14px monospace}.idle{font-size:12px;color:var(--secondary-text-color)}`;
z([
  Z({ attribute: !1 })
], w.prototype, "hass", 2);
z([
  X()
], w.prototype, "machineState", 2);
z([
  X()
], w.prototype, "muted", 2);
w = z([
